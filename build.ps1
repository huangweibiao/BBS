# BBS系统一键打包脚本（PowerShell版本）

Write-Host "====================================" -ForegroundColor Cyan
Write-Host "BBS系统一键打包脚本" -ForegroundColor Cyan
Write-Host "====================================" -ForegroundColor Cyan
Write-Host ""
...
function Clean-Static-Files {
    ...
    $staticDir = "bbs-backend\src\main\resources\static"

    if (Test-Path $staticDir) {
        Write-Host "[5/5] 清理后端静态资源目录中的旧文件: $staticDir" -ForegroundColor Yellow
...
function Copy-Frontend-Build {
    Write-Host "[4/5] 复制前端构建文件到后端..." -ForegroundColor Yellow

    $sourceDir = "bbs-frontend\dist"
    $targetDir = "bbs-backend\src\main\resources\static"
...
function Package-Backend {
    Write-Host "[5/5] 打包后端项目..." -ForegroundColor Yellow

    Set-Location bbs-backend
...
try {
    ...
    Write-Host "====================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "后端JAR包位置: bbs-backend\target\bbs-backend-1.0.0.jar" -ForegroundColor Cyan
    Write-Host "前端静态文件位置: bbs-backend\src\main\resources\static" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "部署命令:" -ForegroundColor Yellow
    Write-Host "  java -jar bbs-backend\target\bbs-backend-1.0.0.jar" -ForegroundColor White
    Write-Host ""
